...
- Biến `Age` được tạo từ việc trích xuất dữ liệu năm sinh (`Year_Birth`) của khách hàng.
...
- `Age` (dữ liệu này được thu thập vào năm 2014-2015, tuy nhiên nếu xét về thời điểm 2014 - 2015 thì với giá trị 1893 thì tuổi sẽ là >100 tuổi thì điều này có thể xảy ra nhưng rất khó để phân khúc khách hàng nên vì vậy chúng ta sẽ loại bỏ các quan sát và ta xem đó là dữ liệu lỗi/outlier).
...
Income_NA_Customer <- dataframe_cleaning %>%
  ...
  summarise(Count = n(),
            ...
            Average_Revenue = mean(Total_Spending),
            Max_Revenue = max(Total_Spending))
kable(Income_NA_Customer, caption = "Bảng doanh thu những người không công khai thu nhập")

``` 

=> Cho thấy rằng những người có dữ liệu Income NA là khách hàng chi tiêu mua sắm cũng khá là cao nên sẽ không bỏ dữ liệu này nếu bỏ sẽ mất đi những khách hàng giá trị cao. Nhưng để tiện cho việc Analyze chúng ta phải cần biến dữ liệu NA này thành dữ liệu số, nên tôi quyết định sẽ điền giá trị thiếu bằng mức thu nhập trung vị.
...
Do tồn tại 1 giá trị cao (outlier) nên tôi sẽ tiến hành lọc dữ liệu đó ra, không phải là bỏ quan sát đó chỉ là nếu để giá trị đó tồn tại thì nó sẽ khiến cho những biểu đồ, chỉ số thống kê bị lệch hẳn qua 1 bên so với những dữ liệu còn lại. (Tôi sẽ xem xét khách hàng này riêng biệt).
...
- Đường đậm:biểu thị cho mức chi tiêu trung bình (median) của các nhóm khách hàng.
...
dataframe_Marketing <- dataframe_Analyze %>%
  mutate(Income_level = case_when(Income < 40000 ~ "Low Income",
        ...
        Income_level = factor(Income_level,level = c("Low Income", 
                                                      "Mid Income", 
                                                      "High Income")),
         Behavior_Type = factor(Behavior_Type, level = c("Organic Buyer","Deal Hunter"))) 
...
Biểu đồ trực quan về những nhóm khách hàng tỷ lệ phản hồi cho chiến dịch tiếp thị sắp tới .
```{r}
# Tạo bộ dữ liệu chứa tổng hợp tỷ lệ phản hồi trung bình theo mức lương, hành vi mua sắm

plot_data <- dataframe_Marketing %>% 
  group_by(Income_level, Behavior_Type) %>%
  summarise(Response_Rate_Campaign = mean(Response) * 100) 
  
ggplot(plot_data, aes(x = Income_level, 
                      y = Response_Rate_Campaign, 
                      fill = Behavior_Type)) +
  geom_col(position = "dodge", width = 0.7) +
  labs(title = "Biểu đồ tỷ lệ phản hồi chiến dịch Marketing",
       subtitle = "Dựa trên phân loại thu nhập, hành vi mua sắm của khách hàng",
       x = "Income levels",
       y = "Tỷ lệ phản hồi (%)")
...
kable(Web_Efficency, caption = "Tỷ lệ chuyển đổi")
